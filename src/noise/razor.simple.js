...
Razor.SimpleNoise.prototype.buildWhiteNoise = function() {
  ...
  for (var i = 0; i < pixels.length; i += 4) {
    pixels[i] = Math.floor(Math.random() * (this.R + 1));//this.R * (Math.floor(Math.random() * 256)) / this.R;
    pixels[i + 1] = Math.floor(Math.random() * (this.G + 1));//this.G * (Math.floor(Math.random() * 256)) / this.G;
    pixels[i + 2] = Math.floor(Math.random() * (this.B + 1));//this.B * (Math.floor(Math.random() * 256)) / this.B;
    pixels[i + 3] = 255;
  }
